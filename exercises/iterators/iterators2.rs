...
pub fn capitalize_words_string(words: &[&str]) -> String {
    words
        .iter() //返回迭代器，迭代器可以遍历字符串切片
        .map(|word| capitalize_first(word)) //map接收闭包,capitalize_first对传入字符串切片首字母大写
        .collect::<Vec<String>>() //callect收集所有String到Vec<String>
        .join("") //join将收集的字符串进行连接,同时""作为空字符串,意味着连接时候字符串中间不插入空格
}
