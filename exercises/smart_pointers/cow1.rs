...
mod tests {
    ...
    */
    ...
    fn reference_mutation() -> Result<(), &'static str> {
        ...
        match abs_all(&mut input) {
            Cow::Owned(_) => Ok(()), //虽然是切片,但是由于传入负数,所以需要修改,所以这里应该匹配Cow::Owned
    ...
    */
    ...
    fn reference_no_mutation() -> Result<(), &'static str> {
        ...
        let mut input = Cow::from(&slice[..]);
        match abs_all(&mut input) {
            Cow::Borrowed(_) => Ok(()), // ✅ 由于传入的非负数,切片并没有获取所有权,所以这里应该匹配Cow::Borrowed
    ...
    */
    ...
    fn owned_no_mutation() -> Result<(), &'static str> {
        ...
        let slice = vec![0, 1, 2];
        let mut input = Cow::from(slice); //所有权发生了变化，所以这里的 input 变成了 owned
        match abs_all(&mut input) {
